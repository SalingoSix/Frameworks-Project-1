...
iShape* cShapeFactory::CreateShape(std::string shapeType, cGameObject* newObj)
{
	...
	if (shapeType == "CRAZYSPHERE")
	{
		newShape = new cCrazySphere;
		AssembleShape(newShape, shapeType, newObj);
	}
	if (newShape != NULL)
...
void cShapeFactory::AssembleShape(iShape* newShape, std::string objType, cGameObject* newObj)
{
	...
	else if (objType == "CRAZYSPHERE")
	{
		cCrazySphere* pCrazy = (cCrazySphere*)newShape;
		pCrazy->pObject = newObj;
		pCrazy->radius = newObj->scale;
		pCrazy->startPos = newObj->position;
	}
}
...
void cShapeFactory::physicsStep(double deltaTime)
{
	for (unsigned int i = 0; i < vec_pShapes.size(); i++)
	{
				...
				if (otherShapeType == 1) // Check for Sphere-Plane collision
				{
					...
					for (int indexT = 0; indexT < thisMesh.numberOfTriangles; indexT++)
					{
						...
						{	//THIS IS A COLLISION
							...
							glm::vec3 reflection = 2.0f * planeNormal * (planeNormal * pCurSphere->getVelocity());
							pCurSphere->setVelocity(pCurSphere->getVelocity() - reflection);
		...
		else if (shapeType == 3) // This shape is a CRAZY sphere
			{
				cCrazySphere* pCurSphere = (cCrazySphere*)pCurShape;
				for (int indexEO = 0; indexEO != vec_pShapes.size(); indexEO++)
				{
					// Don't test for myself
					if (i == indexEO)
					...
					{
						cSphere* pOtherSphere = (cSphere*)pOtherShape;
						if (PenetrationTestSphereSphere((cSphere*)pCurSphere, pOtherSphere))
						{
							float m1, m2, x1, x2;
							glm::vec3 v1, v2, v1x, v2x, v1y, v2y, x(pCurSphere->getPosition() - pOtherSphere->getPosition());
					...
					{
						cCrazySphere* pOtherSphere = (cCrazySphere*)pOtherShape;
						if (PenetrationTestSphereSphere((cSphere*)pCurSphere, (cSphere*)pOtherSphere))
						{
							float m1, m2, x1, x2;
							glm::vec3 v1, v2, v1x, v2x, v1y, v2y, x(pCurSphere->getPosition() - pOtherSphere->getPosition());
